...
/**
 * This class finds the greatest product of four adjacent numbers in the same
 * direction (up, down, left, right, or diagonally) in GRID.
 */
public class Problem11 {
  ...
  public static void main(String[] args) {
    System.out.println(greatestProduct(GRID));
  }

  /**
   * Computes the greatest product of four adjacent numbers in the same
   * direction in GRID.
   */
  public static int greatestProduct(String[] rows) {
    String[][] stringGrid = new String[rows.length][rows.length];
    int[][] grid = new int[rows.length][rows.length];
    int highest = 0;
    for (int i = 0; i < rows.length; i++) {
      stringGrid[i] = rows[i].split(" ");
    }
